...
resource "aws_identitystore_group" "rearadmiral" {
  identity_store_id = local.identity_store_id
  display_name      = "SSOAdmiral"
}

resource "aws_identitystore_group_membership" "rearadmiral" {
  identity_store_id = local.identity_store_id
...
resource "aws_ssoadmin_account_assignment" "base_rearadmiral" {
  instance_arn       = local.sso_instance_arn
  permission_set_arn = aws_ssoadmin_permission_set.rearadmiral_access.arn
...
resource "aws_identitystore_group" "rear_admiral" {
  identity_store_id = local.identity_store_id
  display_name      = "RearAdmiral"
}

resource "aws_identitystore_user" "rear_admiral" {
...
resource "aws_ssoadmin_permission_set" "rear_admiral" {
  name             = "RearAdmiralPermissionSet"
  instance_arn     = local.sso_instance_arn
  session_duration = "PT4H"
}

resource "aws_ssoadmin_managed_policy_attachment" "rear_admiral" {
  instance_arn       = local.sso_instance_arn
